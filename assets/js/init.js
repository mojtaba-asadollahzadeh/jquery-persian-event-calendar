...
var calendar = {
  ...
  startCalendar: function() {
        ...
        var strDate = now.pDate.year + "/" + (now.pDate.month) + "/" + now.pDate.date;
        var yearNumber = now.pDate.year;
        /**
         * Get current month and set as '.current-month' in title
         */
        var monthNumber = now.pDate.month;
        ...
        setMonth(monthNumber,yearNumber);

        function setMonth(set_month,set_year) {
            $('.month').text(GetMonthName(set_month) + ' ' + FarsiNumber(set_year));
            $('.month').attr('data-month', set_month);
            printDateNumber(set_month,set_year);
        }
        ...
        $('.btn-next').on('click', function(e) {
            ...
            if (monthNumber > 11) {
                ...
                setMonth(parseInt(monthNumber) + 1,yearNumber);
            } else {
                setMonth(parseInt(monthNumber) + 1,yearNumber);
            };
        ...
        function getDaysInMonth(month, year) {
            ...
            date =  persianDate([year,month,1]);
            days = 0;
            while (date.pDate.month === month) {
                days++;
                date = persianDate([year,month,days])
            }
            return days-1;
        }
        ...
        function EnglishNumber(number) {
            ...
            for (var i = 0, len = str.length; i < len; i += 1) {
              return_number += ToEnglishNumber(str.charAt(i));
            }
        ...
        function FarsiNumber(number) {
            ...
            return_number = '';
            for (var i = 0, len = str.length; i < len; i += 1) {
              return_number += ToFarsiNumber(parseInt(str.charAt(i)));
            }

            return return_number;
        }

        function printDateNumber(workMonth,workYear) {
            ...
            workDate =  persianDate([workYear,workMonth,1]);
            ...
            for (var i = 1; i <= getDaysInMonth(workMonth, workYear); i++) {

                index = i + offset;
                if (index < 8) {
                    $('tbody.event-calendar tr.1').append('<td date-month="' + workMonth + '" date-day="' + i + '" date-year="' + workYear + '">' + FarsiNumber(i) + '</td>');
                } else if (index < 15) {
                    $('tbody.event-calendar tr.2').append('<td date-month="' + workMonth + '" date-day="' + i + '" date-year="' + workYear + '">' + FarsiNumber(i) + '</td>');
                } else if (index < 22) {
                    $('tbody.event-calendar tr.3').append('<td date-month="' + workMonth + '" date-day="' + i + '" date-year="' + workYear + '">' + FarsiNumber(i) + '</td>');
                } else if (index < 29) {
                    $('tbody.event-calendar tr.4').append('<td date-month="' + workMonth + '" date-day="' + i + '" date-year="' + workYear + '">' + FarsiNumber(i) + '</td>');
                } else if (index < 36 ) {
                    $('tbody.event-calendar tr.5').append('<td date-month="' + workMonth + '" date-day="' + i + '" date-year="' + workYear + '">' + FarsiNumber(i) + '</td>');
                }else{
                    $('tbody.event-calendar tr.6').append('<td date-month="' + workMonth + '" date-day="' + i + '" date-year="' + workYear + '">' + FarsiNumber(i) + '</td>');
                }
            }

            $('tbody.event-calendar td[date-day="' + now.pDate.date + '"][date-month="' + now.pDate.month + '"]')
